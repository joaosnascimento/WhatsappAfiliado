...
export class MercadoLivreAffiliateAdapter implements AffiliateMarketplaceAdapter {
  ...
  public async createAffiliateLink(params: {
    ...
    if (!params.affiliateUrl) {
      throw new Error(
        'Mercado Livre requer vinculação oficial de link de afiliado (ex: meli.la). Obtenha o link no portal de afiliados e associe-o à oferta antes de publicar.'
      );
    }
  ...
  public async testConnection(): Promise<IntegrationTestResult> {
    ...
    steps.push({
      ...
      status: 'SUCCESS',
      message: 'Regra de bloqueio de links comuns ativada. Exige link oficial meli.la validado.',
    });
