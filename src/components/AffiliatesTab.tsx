        ...
        <div className="bg-surface-1 border border-border rounded-lg p-6 shadow-sm space-y-6">
          ...
          <form onSubmit={handleSaveShopee} className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              {shopeeAcc && <button type="button" onClick={async()=>{if(!confirm('Remover as credenciais da Shopee?'))return;try{const r=await apiFetch('/api/accounts/'+encodeURIComponent(shopeeAcc.id),{method:'DELETE'});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Não foi possível remover as credenciais.');toast('success','Credenciais removidas','A Shopee foi desconectada deste workspace.');window.location.reload()}catch(e){toast('error','Shopee',(e as Error).message)}}} className="px-4 py-2 border border-rose-500/20 bg-rose-500/10 text-rose-300 text-sm font-semibold rounded-md">Remover credenciais</button>}
