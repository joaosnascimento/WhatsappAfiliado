    ...
    <div className={card+" p-6 sm:p-8"}>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-4">
          ...
          <button onClick={save} disabled={saving||!url||!instance||( !configured&&!key)} className={button+" bg-brand-500 text-slate-950 hover:bg-brand-400"}>{saving?<><Loader2 className="mr-2 inline h-4 w-4 animate-spin"/>Salvando...</>:configured?'Salvar alterações':'Salvar e continuar'}</button>
        </div>
    ...
    <div className={card+" p-6 sm:p-8"}>
      ...
      <div className="grid items-center gap-8 lg:grid-cols-[1fr_260px]">
        <div>
          ...
          connected?<div className="rounded-lg border border-brand-500/20 bg-brand-500/5 p-6"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-500/15 text-brand-300"><Check className="h-5 w-5"/></div><div><p className="font-semibold text-text">WhatsApp conectado</p><p className="text-sm text-muted">Sua automação já pode enviar ofertas.</p></div></div></div>:
          <><button onClick={connect} disabled={busy} className={button+" bg-white text-slate-950 hover:bg-slate-200"}>{busy?<><Loader2 className="mr-2 inline h-4 w-4 animate-spin"/>Gerando QR Code...</>:<><QrCode className="mr-2 inline h-4 w-4"/>Gerar QR Code</>}</button>{statusError&&<div className="mt-4 flex gap-2 rounded-md border border-red-500/20 bg-red-500/5 p-3 text-sm text-red-300"><CircleAlert className="h-4 w-4 shrink-0"/>{statusError}</div>}<ol className="mt-5 space-y-2 text-sm text-muted"><li><b className="text-text">1.</b> Abra o WhatsApp no celular.</li><li><b className="text-text">2.</b> Vá em <span className="text-slate-200">Aparelhos conectados → Conectar aparelho</span>.</li><li><b className="text-text">3.</b> Aponte a câmera para o código.</li></ol></>}
        </div>
