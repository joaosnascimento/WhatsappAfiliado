      ...
      <div className="grid items-center gap-8 lg:grid-cols-[1fr_260px]">
        <div>
          ...
          {configured && connected && (
            <div className="rounded-lg border border-brand-500/20 bg-brand-500/5 p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-500/15 text-brand-300"><Check className="h-5 w-5"/></div>
                  <div><p className="font-semibold text-text">WhatsApp conectado</p><p className="text-sm text-muted">Sua automação já pode enviar ofertas.</p></div>
                </div>
                <button onClick={disconnect} disabled={busy} className={button+" border border-red-500/30 text-red-300 hover:bg-red-500/10"}>{busy?<Loader2 className="h-4 w-4 animate-spin"/>:'Desconectar'}</button>
              </div>
            </div>
          )}
          ...
          {configured && !connected && (
            <>
              ...
              <ol className="mt-5 space-y-2 text-sm text-muted">
                <li><b className="text-text">1.</b> Abra o WhatsApp no celular.</li>
                <li><b className="text-text">2.</b> Vá em <span className="text-text">Aparelhos conectados → Conectar aparelho</span>.</li>
                <li><b className="text-text">3.</b> Aponte a câmera para o código.</li>
              </ol>
            </>
          )}
