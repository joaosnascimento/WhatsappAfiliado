    ...
    <div className={card+" p-6 sm:p-8"}>
      ...
      <div className="grid items-center gap-8 lg:grid-cols-[1fr_260px]">
        <div>
          ...
          <><button onClick={connect} disabled={busy} className={button+" bg-white text-bg hover:bg-slate-200"}>{busy?<><Loader2 className="mr-2 inline h-4 w-4 animate-spin"/>Gerando QR Code...</>:<><QrCode className="mr-2 inline h-4 w-4"/>Gerar QR Code</>}</button>{statusError&&<div className="mt-4 flex gap-2 rounded-md border border-red-500/20 bg-red-500/5 p-3 text-sm text-red-300"><CircleAlert className="h-4 w-4 shrink-0"/>{statusError}</div>}<ol className="mt-5 space-y-2 text-sm text-muted"><li><b className="text-text">1.</b> Abra o WhatsApp no celular.</li><li><b className="text-text">2.</b> Vá em <span className="text-text">Aparelhos conectados → Conectar aparelho</span>.</li><li><b className="text-text">3.</b> Aponte a câmera para o código.</li></ol></>}
        </div>
