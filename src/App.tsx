  ...
  return (
    <div className="min-h-screen bg-bg text-text flex flex-col font-sans selection:bg-brand-500 selection:text-bg">
      ...
      <main className="flex-1 w-full px-4 py-6 sm:px-6 lg:px-10 lg:py-8"><div className="animate-page-in">
        ...
        {activeTab === 'coupons' && <CouponsTab apiFetch={apiFetch} />}\n        {activeTab === 'offers' && <OffersTab offers={offers} onLiveSearch={handleLiveSearch} onManualAddMercadoLivre={handleManualMercadoLivre} onOpenAssociateModal={(offer)=>setAssociateModalOffer(offer)} onOpenAiMessageModal={(offer)=>setAiModalOffer(offer)} onDeleteOffer={handleDeleteOffer} />}
        {activeTab === 'destinations' && <DestinationsTab destinations={destinations} onAddDestination={handleAddDestination} apiFetch={apiFetch} onChanged={loadData} />}
